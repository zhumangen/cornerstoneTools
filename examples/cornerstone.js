/*! cornerstone - v0.9.0 - 2016-05-26 | (c) 2014 Chris Hafey | https://github.com/chafey/cornerstone */
...
(function ($, cornerstone) {
    ...
    function renderCompositeImage(enabledElement, invalidated) {
        ...
        firstImage = enabledElement.layers[0].image;

        enabledElement.layers.forEach(function(layer, index) {
            context.save();

            if (index === 0 && layer.viewport) {
                firstViewport.scale = layer.viewport.scale;
                firstViewport.translation.x = layer.viewport.translation.x;
                firstViewport.translation.y = layer.viewport.translation.y;
            }

            if (layer.options.visible === false || layer.options.opacity === 0) {
                return;
            }
            ...
            if (!layer.viewport) {
                ...
                enabledElement.viewport = {
                    translation: {
                        x: layer.viewport.translation.x,
                        y: layer.viewport.translation.y
                    },
                    scale: layer.viewport.scale
                };

                if (index === 0) {
                    viewportRatio[layer.layerId] = 1;
                } else {
                    viewportRatio[layer.layerId] = layer.viewport.scale / firstViewport.scale;
                }
            }

            if (enabledElement.syncViewports === true && index !== 0) {
                layer.viewport.translation.x = firstViewport.translation.x * layer.image.width / firstImage.width;
                layer.viewport.translation.y = firstViewport.translation.y * layer.image.height / firstImage.height;
                layer.viewport.scale = firstViewport.scale * viewportRatio[layer.layerId];
            }
        ...
        var topLayer = enabledElement.layers[0];
        enabledElement.viewport = topLayer.viewport;
        enabledElement.image = topLayer.image;
    }
...
(function (cornerstone) {
    ...
    function removeLayer(element, layerId) {
        ...
        if (index !== undefined) {
            enabledElement.layers.splice(toRemove, 1);
...
(function (cornerstone) {
    ...
    function addPixelOverlay(layer, invalidated) {
        if(layer === undefined) {
            throw "drawImage: layer parameter must not be undefined";
        }
    ...
    // Module exports
    cornerstone.addPixelOverlay = addPixelOverlay;
